        ...
        init : function(options) {
            var settings = $.extend( {
                ...
                beforeSubmit: function(wizardObj) {
                    ...
                    var cds_officer = $('#cds_officer').val();
		    var orderNo     = $('#oderNo').val();	
                    var destination = $('#lto').val();
                    var origin      = $('#lfrom').val();
                   
                    var formData = $('#myWizard').serializeArray();
                    
                    formData.push({"name":"cds_officer", "value" : cds_officer});
                    formData.push({"name":"origin", "value" : origin });
                    formData.push({"name":"destination", "value" : destination});
                    formData.push({"name":"orderNo", "value" : orderNo});


                    console.log(JSON.stringify(formData));

                    
                    $('#myWizard').css('opacity', '0.1');
                    $('#loader').show();
                    $.post('add', formData, function(data){
                        $('#loader').hide();
                        $('#loader2').fadeIn(3000, function(){
                            $('#myWizard').css('opacity', '1');
                            $('#content').html('');
                            $(this).fadeOut();
                        });
                    });
